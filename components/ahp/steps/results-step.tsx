...
export default function ResultsStep() {
  ...
  if (!criteriaPairwiseComplete || !alternativesPairwiseComplete) {
    return (
      <Alert>
        ...
        <AlertDescription>
          Please complete all pairwise comparisons before viewing the results.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  const hasInconsistentJudgments =
    ...
    alternativesPairwise
      .filter(
        (matrix) =>
          !criteria.find((c) => c.id === matrix.criterionId)?.isQuantitative
      )
      .some(
        (matrix) => matrix.consistencyRatio > 0.1 && alternatives.length > 2
      );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 mb-6">
          Below are the final results of your AHP analysis, showing the ranking
          of alternatives based on your criteria weights and judgments. 
          ...
          <div>
            {hasInconsistentJudgments && (
              <Alert variant="destructive" className="mb-4">
                ...
                <AlertDescription>
                  Some of your pairwise comparisons have consistency ratios
                  greater than 0.1, which may affect the reliability of your
                  results. Consider going back to revise your comparisons.
                </AlertDescription>
              </Alert>
            )}
          </div>
        ...
        <div>
          <h3 className="font-medium text-green-700">Textual Result</h3>
          <p className="text-green-600 text-sm mt-1">
          The application of the AHP method resulted in a clear ranking of
                the alternatives, with <span className="font-bold">{` ${
                  alternatives.find(
                    (a) => a.id === finalRanking[0].alternativeId
                  )?.name
                } `}</span> securing the highest
                position. Its cumulative score of <span className="font-bold">{` ${formatNumber(
          ...
          <TableBody>
            {criteria.map((criterion, index) => (
              <TableRow key={criterion.id}>
                ...
                <TableCell>
                  {formatNumber(
                    criteriaPairwise?.priorityVector?.[index] ?? 0,
                    4
                  )}
                </TableCell>
              </TableRow>
              ...
              return (
                <TableRow key={alternative.id}>
                  ...
                  {criteria.map((criterion, criterionIndex) => {
                    const matrix = alternativesPairwise.find(
                      (m) => m.criterionId === criterion.id
                    );
      ...
      {/* <div className="bg-blue-50 p-4 rounded-md">
        <h3 className="font-medium text-blue-700">Final Result</h3>
        <p className="text-blue-600 text-sm mt-1">
          Based on the  calculation using AHP method, the highest rank is the alternatives
        </p>
      </div> */}
      ...
      <div className="bg-green-50 p-4 rounded-md flex items-start">
        ...
        <div>
          ...
          <p className="text-green-600 text-sm mt-1">
            Your AHP analysis is complete. You can now use these results to make
            an informed decision. If you want to make changes, you can go back
            to previous steps and update your inputs.
          </p>
        </div>
      </div>
    </div>
