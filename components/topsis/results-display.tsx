        ...
        <TabsContent value="matrices" className="space-y-6 mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>The original input values for each alternative and criterion</CardDescription>
            </CardHeader>
            <CardContent>
              <MatrixTable
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Values normalized using vector normalization</CardDescription>
            </CardHeader>
            <CardContent>
              <MatrixTable
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Normalized values multiplied by their respective weights</CardDescription>
            </CardHeader>
            <CardContent>
              <MatrixTable
        ...
        <TabsContent value="ideals" className="space-y-6 mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Positive ideal (best) and negative ideal (worst) solutions</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {results.criteria.map((criterion, index) => (
                      <TableHead key={`ideal-header-${index}`}>{criterion}</TableHead>
                    ))}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <TableRow>
                    <TableCell className="font-medium">Positive Ideal (A+)</TableCell>
                    {results.positiveIdeal.map((value, index) => (
                      <TableCell key={`positive-${index}`}>{value.toFixed(4)}</TableCell>
                    ))}
                  </TableRow>
                  <TableRow>
                    <TableCell className="font-medium">Negative Ideal (A-)</TableCell>
                    {results.negativeIdeal.map((value, index) => (
                      <TableCell key={`negative-${index}`}>{value.toFixed(4)}</TableCell>
                    ))}
                  </TableRow>
                </TableBody>
              </Table>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Type of each criterion (benefit or cost)</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    {results.criteria.map((criterion, index) => (
                      <TableHead key={`type-header-${index}`}>{criterion}</TableHead>
                    ))}
                  </TableRow>
                </TableHeader>
                <TableBody>
        ...
        <TabsContent value="distances" className="space-y-6 mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Distances from each alternative to the ideal solutions</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {results.alternatives.map((alt, index) => (
                    <TableRow key={`distance-${index}`}>
                      ...
                      <TableCell>{results.distances.positive[index].toFixed(4)}</TableCell>
                      <TableCell>{results.distances.negative[index].toFixed(4)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <TabsContent value="ranking" className="space-y-6 mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Final scores and ranking of alternatives</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {results.ranking.map((rank, index) => (
                    <TableRow key={`rank-${index}`} className={index === 0 ? "bg-primary/10" : ""}>
              ...
              <div className="mt-6 p-4 bg-primary/10 rounded-lg">
                <p className="font-medium">
                  Best Alternative: {results.alternatives[results.ranking[0].index]} with score{" "}
                  {results.ranking[0].score.toFixed(4)}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Visual comparison of alternatives by their scores</CardDescription>
              </CardHeader>
              <CardContent>
                <BarChart labels={results.alternatives} scores={results.scores} bestIndex={results.ranking[0].index} />
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Radar chart comparing alternatives across criteria</CardDescription>
              </CardHeader>
              <CardContent>
                <RadarChart
          ...
          {matrix.map((row, rowIndex) => (
            <TableRow key={`row-${rowIndex}`}>
              ...
              {row.map((value, colIndex) => (
                <TableCell key={`cell-${rowIndex}-${colIndex}`}>{value.toFixed(4)}</TableCell>
              ))}
            </TableRow>
          ))}
        </TableBody>
